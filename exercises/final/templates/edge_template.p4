...
const bit<32> my_host_ip = $my_host_ip; // 8w10++8w0++8w1++8w1;
const bit<48> my_host_mac = $my_host_mac; // 8w8++8w0++8w0++8w0++8w1++8w0x11;

/*
 * Next hop address (should be another switch), filled in by generator
 */
 
const bit<48> next_hop_mac = $next_hop_mac; // 8w8++8w0++8w0++8w0++8w2++8w0;
