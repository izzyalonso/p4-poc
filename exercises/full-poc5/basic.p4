...
bool parseCustom(in headers hdr, in bit<16> dataLength) {
    return hdr.ipv4.srcAddr == h2_ip && dataLength > 0;
}
...
parser MyParser(packet_in packet,
    ...
    state parse_tcp {
        ...
        meta.dataLength = meta.tcpLength-((bit<16>)hdr.tcp.dataOffset*4);
        packet.extract(hdr.tcp_options, (bit<32>)(((bit<32>)hdr.tcp.dataOffset-5)*32));
        transition select(parseCustom(hdr, meta.dataLength)) {
            true: parse_custom;
    ...
    state generate_custom {
        hdr.custom.full = { hdr.ipv4.srcAddr, (bit<32>)meta.dataLength };
        transition accept;
    }
}
...
bool emitCustom(in headers hdr, in metadata meta) {
    return hdr.ipv4.dstAddr == h2_ip && meta.dataLength > 0;
}
...
control MyEgress(inout headers hdr,
                 ...
                 inout standard_metadata_t standard_metadata) {
    apply {
        if (emitCustom(hdr, meta)) {
            hdr.ipv4.totalLen = hdr.ipv4.totalLen+8;
            meta.tcpLength = meta.tcpLength+8;
            meta.willEmitCustom = 1w1;
        } else {
            custom_empty_t empty = { };
            hdr.custom.empty = empty;
            meta.willEmitCustom = 1w0;
        }
    }
}
...
control MyComputeChecksum(inout headers hdr, inout metadata meta) {
    apply {
        ...
        update_checksum_with_payload(
            hdr.tcp.isValid() && meta.willEmitCustom == 1w0,
            {
                hdr.ipv4.srcAddr,
                hdr.ipv4.dstAddr,
                8w0,
        ...
        update_checksum_with_payload(
            hdr.tcp.isValid() && meta.willEmitCustom == 1w1,
            {
                hdr.ipv4.srcAddr,
                hdr.ipv4.dstAddr,
                8w0,
